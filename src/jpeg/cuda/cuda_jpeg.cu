...
namespace jpeg {
	namespace cuda {
		...
		namespace kernel {
			...
			namespace ver2 {
				...
				__global__ void ConvertRGBToYUV(const byte* rgb, byte* yuv_result,
					...
					const int dst_y_index = table[pix_index];
				...
				__global__ void ConvertYUVToRGB(const byte* yuv, byte* rgb_result,
					...
					const int src_index = table[pix_index];

					const int y = yuv[src_index];
					//const int u = yuv[src_u_id] - 128;
					//const int v = yuv[src_v_id] - 128;
		 ...
		 * @param table
		 */
		void CreateConversionTable(size_t width, size_t height, size_t block_width, size_t block_height, int *table) {
			...
			for (int block_y = 0; block_y < img_block_y_num; ++block_y) {
				for (int block_x = 0; block_x < img_block_x_num; ++block_x) {
					const int block_id = block_x + block_y * img_block_x_num;
					const int src_block_start_index = block_y * width * block_height + block_x * block_width;
					const int dst_block_start_y_index = block_id * img_block_size * 3 / 2;
					const int dst_block_start_u_index = dst_block_start_y_index + img_block_size;
					const int dst_block_start_v_index = dst_block_start_u_index + img_block_size / 4;

					for (int mcu_y = 0; mcu_y < mcu_block_y_num; ++mcu_y) {
						for (int mcu_x = 0; mcu_x < mcu_block_x_num; ++mcu_x) {
							const int mcu_id = mcu_x + mcu_y * mcu_block_x_num;
							const int src_mcu_start_index = src_block_start_index + mcu_y * width * 16 + mcu_x * 16;
							const int dst_mcu_y_start_index = dst_block_start_y_index + mcu_id * 256;
							const int dst_mcu_u_start_index = dst_block_start_u_index + mcu_id * 64;
							const int dst_mcu_v_start_index = dst_block_start_v_index + mcu_id * 64;
							...
							for (int pix_y = 0; pix_y < 16; ++pix_y) {
								for (int pix_x = 0; pix_x < 16; ++pix_x) {
									...
									const int mcu_id_y = pix_y / 8; // 0,1
									const int block_8x8_id = mcu_id_x + 2 * mcu_id_y; // 0-3
									const int dst_mcu_index = pix_x % 8 + (pix_y % 8) * 8; // 0-63

									// RGB画像のピクセルインデックス
									const int src_index = src_mcu_start_index + pix_x + pix_y * width;
									// YUVの書き込みインデックス
									const int dst_index = dst_mcu_y_start_index + block_8x8_id * 64 + dst_mcu_index;

									table[src_index] = dst_index;

									if (pix_x % 2 == 0 && pix_y % 2 == 0) {
										const int x = pix_x / 2, y = pix_y / 2;
										const int dst_u_index = dst_mcu_u_start_index + x + y * 8;
										const int dst_v_index = dst_mcu_v_start_index + x + y * 8;
									}

									printf("CreateTableCPU, src_id, %d, dst_id, %d\n", src_index, table[src_index]);
								}
							}
						}
					}
